...
import { GraphQLObjectType, GraphQLNonNull, GraphQLList, print, graphql } from 'graphql';
import { mapSchema } from '@graphql-tools/utils';
import { makeExecutableSchema } from '@graphql-tools/schema';
import { getAdminMetaSchema, createAdminUIServer } from '@keystone-next/admin-ui/system';
import { sessionSchema, createSessionContext } from '../session/dist/keystone.esm.js';
import { getItem, getItems, createItem, createItems, updateItem, updateItems, deleteItem, deleteItems } from '@keystone-next/server-side-graphql-client-legacy';
...
function createKeystone(config, dotKeystonePath, migrationAction, prismaClient) {
  ...
  if (db.adapter === 'knex') {
    ...
    adapter = new PrismaAdapter(_objectSpread(_objectSpread({
...
const addApolloServer = ({
  ...
  server.use(graphqlUploadExpress()); // FIXME: Support custom API path via config.graphql.path.
...
const createExpressServer = async (config, graphQLSchema, createContext, dev, projectAdminPath, isVerbose = true) => {
  ...
  if ((_config$ui = config.ui) !== null && _config$ui !== void 0 && _config$ui.isDisabled) {
    ...
    server.use(await createAdminUIServer(config.ui, createContext, dev, projectAdminPath, sessionStrategy));
  }
...
export { createExpressServer as a, createApolloServerMicro as b, createSystem as c, createKeystone as d, initConfig as i };
