...
async function deployMigrations(dbUrl, schemaPath) {
  return withMigrate(dbUrl, schemaPath, async migrate => {
    ...
    if (editedMigrationNames.length > 0) {
      console.info(`${chalk__default['default'].yellow('WARNING The following migrations have been modified since they were applied:')}
${editedMigrationNames.join('\n')}`);
    ...
    if (migrationIds.length === 0) {
      console.log(chalk__default['default'].greenBright(`No pending migrations to apply.`));
    } else {
      console.log(`The following migration${migrationIds.length > 1 ? 's' : ''} have been applied:\n\n${printFilesFromMigrationIds(migrationIds)}

${chalk__default['default'].greenBright('All migrations have been successfully applied.')}`);
    }
  });
...
async function resetDatabaseWithMigrations(dbUrl, schemaPath) {
  return withMigrate(dbUrl, schemaPath, async migrate => {
    ...
    console.log(`${chalk__default['default'].green('Database reset successful')}`);
...
async function ensureDatabaseExists(dbUrl, schemaDir) {
  ...
  const result = await sdk.createDatabase(dbUrl, schemaDir);
...
class PrismaAdapter extends keystoneLegacy.BaseKeystoneAdapter {
  constructor(config = {}) {
    super(...arguments);
  ...
  async _prepareSchema(rels) {
    ...
    this.schemaPath = path__default['default'].join(prismaPath, 'schema.prisma');
    this.clientPath = path__default['default'].resolve(`${prismaPath}/${clientDir}`);
  ...
  async deploy(rels) {
    ...
    await deployMigrations(this._url(), path__default['default'].resolve(this.schemaPath));
  }
  ...
  async _runMigrations({
    ...
    if (this.migrationMode === 'prototype') {
      ...
      await runPrototypeMigrations(this._url(), prismaSchema, path__default['default'].resolve(this.schemaPath));
    } else if (this.migrationMode === 'dev') {
      // Generate and apply a migration if required.
      await devMigrations(this._url(), prismaSchema, path__default['default'].resolve(this.schemaPath));
  ...
  async _writePrismaSchema({
    ...
    fs__default['default'].mkdirSync(this.clientPath, {
      recursive: true
    }); // Write prisma file

    fs__default['default'].writeSync(fs__default['default'].openSync(this.schemaPath, 'w'), prismaSchema);
  }
  ...
  async _generatePrismaSchema({
    ...
    const models = Object.values(this.listAdapters).map(listAdapter => {
      const scalarFields = utilsLegacy.flatten(listAdapter.fieldAdapters.filter(f => !f.field.isRelationship).map(f => f.getPrismaSchema()));
      const relFields = [...utilsLegacy.flatten(listAdapter.fieldAdapters.map(({
        field
      ...
      })), ...utilsLegacy.flatten(rels.filter(({
        right
      }) => !right).filter(({
        left
        ...
        tableName
      }) => [`from_${listKey}_${path} ${listKey}[] @relation("${tableName}", references: [id])`])), ...utilsLegacy.flatten(rels.filter(({
        right
      }) => !right).filter(({
        left
      }) => left.refListKey === listAdapter.key).filter(({
      ...
      const indexes = utilsLegacy.flatten(listAdapter.fieldAdapters.map(({
        field
      }) => field).filter(f => f.isRelationship).map(f => {
        const r = rels.find(r => r.left === f || r.right === f);
        const isLeft = r.left === f;
    ...
    const enums = utilsLegacy.flatten(Object.values(this.listAdapters).map(listAdapter => utilsLegacy.flatten(listAdapter.fieldAdapters.filter(f => !f.field.isRelationship).filter(f => f.path !== 'id').map(f => f.getPrismaEnums()))));
    const header = `
  ...
  async postConnect({
    ...
    if (this.config.dropDatabase &&         "production" !== 'production') {
  ...
  async dropDatabase() {
    if (this.migrationMode === 'prototype') {
      ...
      resetDatabaseWithMigrations(this._url(), path__default['default'].resolve(this.schemaPath));
    }
  }
...
class PrismaListAdapter extends keystoneLegacy.BaseListAdapter {
  ...
  _include() {
    ...
    const include = utilsLegacy.defaultObj(this.fieldAdapters.filter(({
      isRelationship
  ...
  async _create(_data) {
    return this.model.create({
      data: utilsLegacy.mapKeys(_data, (value, path) => this.fieldAdaptersByPath[path] && this.fieldAdaptersByPath[path].isRelationship ? {
  ...
  async _update(id, _data) {
    ...
    return this.model.update({
      ...
      data: utilsLegacy.mapKeys(_data, (value, path) => {
...
class PrismaFieldAdapter extends keystoneLegacy.BaseFieldAdapter {
  constructor() {
    super(...arguments);
  }
  ...
  equalityConditions(dbPath, f = utilsLegacy.identity) {
    return {
      [this.path]: value => ({
        [dbPath]: {
          equals: f(value)
  ...
  equalityConditionsInsensitive(dbPath, f = utilsLegacy.identity) {
    return {
      [`${this.path}_i`]: value => ({
        [dbPath]: {
          equals: f(value),
  ...
  inConditions(dbPath, f = utilsLegacy.identity) {
    return {
  ...
  orderingConditions(dbPath, f = utilsLegacy.identity) {
    return {
      [`${this.path}_lt`]: value => ({
        [dbPath]: {
          lt: f(value)
  ...
  stringConditions(dbPath, f = utilsLegacy.identity) {
    return {
  ...
  stringConditionsInsensitive(dbPath, f = utilsLegacy.identity) {
    return {
...
exports.PrismaFieldAdapter = PrismaFieldAdapter;
exports.PrismaListAdapter = PrismaListAdapter;
