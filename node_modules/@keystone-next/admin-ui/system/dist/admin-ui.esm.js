...
import { w as writeAdminFiles, s as serializePathForImport } from '../../dist/index-2a5b787b.esm.js';
import url from 'url';
...
async function writeAdminFile(file, projectAdminPath) {
  ...
  if (file.mode === 'copy') {
    ...
    await fs.ensureDir(Path__default.dirname(outputFilename)); // TODO: should we use copyFile or copy?
...
const generateAdminUI = async (config, graphQLSchema, keystone, projectAdminPath) => {
  ...
  await Promise.all(adminFiles.filter(x => !uniqueFiles.has(Path__default.normalize(x.outputPath))).map(file => writeAdminFile(file, projectAdminPath))); // Add files to pages/ which point to any files which exist in admin/pages
